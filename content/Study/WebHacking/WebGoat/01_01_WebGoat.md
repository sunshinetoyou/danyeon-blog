...
또한, Payload에서 Requests 시에 같이 보내지는 데이터를 보면, 다음과 같이 `magic_num`(정답)이 같이 보내짐을 확인할 수 있다. 
이는 우리가 입력창에 넣은 `magic_answer`과 비교되어 결과가 계산됨을 유추할 수 있다.  
...
페이지가 로드되면, 1부터 100까지의 랜덤한 수를 magic_num으로 결정짓고
